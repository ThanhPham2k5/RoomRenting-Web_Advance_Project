  ...
  <script>
    ...
    async function getPost(sortCondition, filterCondition, searchCondition, page, account_id, token) {
      try {
        ...
        const response = await fetch("http://127.0.0.1:8000/api/favorites?per_page=10&include=post.postImages,account&filter[post.status]=completed&filter[account.id]=" + account_id + "&sort=" + sortCondition + filterCondition + searchCondition + "&page=" + page, {
          method: "GET",
          headers: {
            "Accept": "application/json",
            "Authorization": "Bearer " + token
