  ...
  <script>
    ...
    async function getPost(sortCondition, filterCondition, searchCondition, page, account_id, token) {
      try {
        ...
        const response = await fetch("http://127.0.0.1:8000/api/posts/recommendations/" + account_id + "?per_page=10&filter[status]=completed&include=postImages,favorites.account&sort=" + sortCondition + filterCondition + searchCondition + "&page=" + page, {
          method: "GET",
          headers: {
            "Accept": "application/json",
            "Authorization": "Bearer " + token
