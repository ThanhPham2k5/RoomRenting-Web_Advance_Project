...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href='<?php echo BASE_URL . "/css/client/pages/favourite.css" ?>'/>
    ...
    <title>Favourite Post Page | RoomRenting</title>
  </head>
  <body>
  ...
  <script>
    ...
    document.querySelector(".filter-apply").addEventListener("click", async (e) => {
      ...
      if(document.querySelector(".filter-square-number").value.trim() && squareRegex.test(document.querySelector(".filter-square-number").value.trim()) && document.querySelector(".filter-square-number").value.trim() > 0) {
        ...
        filterCondition += "&filter[post.area]=" + area
      }
      ...
      if(document.querySelector(".filter-occupants-number").value.trim() && numbRegex.test(document.querySelector(".filter-occupants-number").value.trim()) && document.querySelector(".filter-occupants-number").value.trim() > 0) {
        filterCondition += "&filter[post.maxOccupants][eq]=" + document.querySelector(".filter-occupants-number").value.trim()
      }
            ...
            html += `
              <div class="post">
                ...
                <a href='<?php echo BASE_URL ?>/pages/client/detail-post.php?post_id=${post.post.id}' class="post-body">
                  <img
                    src='http://backend.test${post.post.postImages[0].imagePostUrl}'
                    alt="post.png"
                    class="post-img"
                  />
                  ...
                  <div class="post-info">
                    <h3 class="post-price">${formatPrice(post.post.price)} VND/tháng</h3>
